...
def df_conso_brute_viz():
    st.subheader('CSV Consomation électricité et gaz')
...
def visualization_conso_par_filiere_operateur():
    ...
    axe_x = st.selectbox(
        "Sélectionner une colonne pour l'axe x ", colonnes_axe_x)
    ...
    axe_y = st.selectbox(
        "Sélectionner une colonne pour l'axe y", colonnes_axe_y)
    data_for_chart = df_final[[axe_x, axe_y]].copy()

    # Conversion des données en valeurs numériques
    data_for_chart[axe_y] = pd.to_numeric(
        data_for_chart[axe_y], errors='coerce')
...
def geoloc_viz():
    st.subheader(
        'Visualisation des consommations totals(électricité/gaz) par commune')
    df_cleaned = df_final.dropna(subset=['latitude', 'longitude'])
    m = folium.Map(location=[48.8566, 2.3522], zoom_start=5)
    # Ajouter des marqueurs pour chaque station
    for index, row in df_cleaned.iterrows():
        popup_text = f"Consommation totale (MWh): {row['Consommation totale (MWh)']}"
...
def visualization_conso_total_par_année():
    st.subheader("Visualisation de la consommation total par année")
    fig = px.bar(df_final, x=df_final["Année"],
                 y=df_final["Consommation totale (MWh)"])
    st.plotly_chart(fig)
